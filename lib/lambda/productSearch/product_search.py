...
def create_product_embedding(base64Image, generatedImageLabel):
    ...
    # Add inputText if label is valid
    if isinstance(generatedImageLabel, str) and generatedImageLabel.strip():
        payload["inputText"] = generatedImageLabel.strip()
        
    # Add inputImage if base64 is valid
    if isinstance(base64Image, str) and base64Image.strip():
        payload["inputImage"] = base64Image.strip()
        
    bodyWithImageAndDescription =   json.dumps(payload)
...
def do_text_search(body_json):
    query = body_json['content']
    ...
    # Create multimodal embedding
    embedding = create_product_embedding("", query)
    print("Created multimodal embedding")
